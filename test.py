...
_SENSITIVE_KEYS = {"client_secret", "card", "number", "cvc", "token", "access_token", "refresh_token"}

def _sanitize_response(obj):
    """递归脱敏响应中的敏感字段"""
    if isinstance(obj, dict):
        return {
            k: ("***" if k in _SENSITIVE_KEYS else _sanitize_response(v))
...
def _log_response(resp: requests.Response, tag: str = ""):
    """记录 HTTP 响应详情（敏感字段已脱敏）"""
    _log_raw(f"<<< RESPONSE {tag}  status={resp.status_code}")
    try:
        body = _sanitize_response(resp.json())
...
def _gen_canvas_fp() -> str:
    """生成随机但结构合理的 canvas 指纹（53 位二进制串）"""
    # 前缀保持常见模式，后段随机化
    prefixes = ["01001001011111111011111011111110",
                "01001001011111101011111011111110",
                "01001001011111111011111011111100",
                "01001001011111111011110011111110",
                "01001001011111101011110011111110"]
    suffix_len = 53 - len(prefixes[0])
    return random.choice(prefixes) + ''.join(random.choice('01') for _ in range(suffix_len))

def _gen_audio_fp() -> str:
    """生成随机 audio 指纹（32 位 hex）"""
...
def register_fingerprint(http: "requests.Session") -> tuple[str, str, str]:
    ...
    canvas_fp = _gen_canvas_fp()
    audio_fp = _gen_audio_fp()
    ...
    def _build_mouse(source: str) -> dict:
        # 生成更真实的鼠标移动时序数据（递增的时间间隔）
        mouse_data = []
        t = random.randint(50, 200)
        for _ in range(random.randint(8, 15)):
            mouse_data.append(t)
            t += random.randint(10, 150)
        return {
            "muid": muid, "sid": sid,
...
def send_telemetry_batch(
    ...
    for i, evt in enumerate(events):
        ...
        # 真实浏览器事件间隔: 100ms-1.5s，首事件后稍长
        time.sleep(random.uniform(0.1, 0.5) if i == 0 else random.uniform(0.3, 1.5))
...
def confirm_payment(
    ...
    # 使用传入的 stripe_ver 参数（confirm 阶段需要完整版本号）
    ver = stripe_ver or STRIPE_VERSION_FULL
    ...
    data = {
        ...
        "client_attribution_metadata[elements_session_config_id]": ctx.get("config_id", str(uuid.uuid4())),
...
def run(checkout_input: str, card_index: int = 0, config_path: str = "config.json", manual_token: str = ""):
    ...
    card["name"] = f"{random.choice(_FIRST_NAMES).title()} {random.choice(_LAST_NAMES).title()}"

    # 生成更真实的邮箱: firstname + 分隔符 + 数字后缀
    _EMAIL_DOMAINS = ["gmail.com", "yahoo.com", "outlook.com", "hotmail.com", "icloud.com"]
    first = card["name"].split()[0].lower()
    last = card["name"].split()[-1].lower()
    sep = random.choice([".", "_", ""])
    suffix = str(random.randint(1, 999)) if random.random() > 0.3 else ""
    patterns = [
        f"{first}{sep}{last}{suffix}",
        f"{first}{suffix}{sep}{last[0]}",
        f"{first[0]}{last}{suffix}",
        f"{first}{sep}{last}{random.randint(70, 99)}",
    ]
    ...
    # 网络重试策略: 连接错误/超时/502/503/504 自动重试 3 次
    ...
    # 模拟用户输入卡号的时间（真实用户需要 8-20 秒填写卡号/日期/CVC）
    time.sleep(random.uniform(8.0, 18.0))

    # 更新 time_on_page 为实际经过时间
    init_ctx["time_on_page"] = int(time.time() * 1000) - init_ctx.get("page_load_ts", int(time.time() * 1000))
