...
class SubmitPhoneAndCodeHandlerTest(unittest.TestCase):
    ...
    def test_returns_false_when_sms_api_missing(self):
        from src.orchestration.handlers import submit_phone_and_code
        runtime, _, _ = self._make_runtime(sms_api=False)
        # 用 False 占位让 None-check 命中（sms_api is None 分支）
        runtime.sms_api = None
        self.assertFalse(submit_phone_and_code(runtime, None))

    def test_returns_false_when_otp_timeout(self):
        from src.orchestration.handlers import submit_phone_and_code
        runtime, _, _ = self._make_runtime(sms_code=None)
        self.assertFalse(submit_phone_and_code(runtime, None))

    def test_happy_path_fills_phone_then_otp_returns_true(self):
        from src.orchestration.handlers import submit_phone_and_code
        runtime, page, sms_api = self._make_runtime(phone="+14155551212",
                                                     order_id="ord-1", sms_code="123456")
        self.assertTrue(submit_phone_and_code(runtime, None))
        # 检查 sms_api.get_code 用 order_id 调过一次
        sms_api.get_code.assert_called_once()
        args, _ = sms_api.get_code.call_args
        self.assertEqual(args[0], "ord-1")
        # page.fill 至少被调过 2 次（手机号一次 + OTP 一次）
        self.assertGreaterEqual(page.fill.call_count, 2)
        # page.click 也被尝试过（即使两次提交时可能 except 吞掉，调用次数仍要 >=1）
        self.assertGreaterEqual(page.click.call_count, 1)
